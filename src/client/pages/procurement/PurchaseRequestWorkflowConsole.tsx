...
export default function PurchaseRequestWorkflowConsole() {
  ...
  const [formData, setFormData] = useState({
    ...
    estimatedCost: 1500,
    priority: 'MEDIUM',
    reason: 'Inventory threshold reorder trigger',
  });
  ...
  const handleCreateRequest = async () => {
    try {
      ...
      toast.success('Purchase Request submitted for manager approval!');
      setOpenModal(false);
      fetchData();
    } catch (err: any) {
    ...
    <Box sx={{ p: 3, background: '#0b0f19', minHeight: '100vh', color: '#f8fafc' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Box>
          ...
          >
            <AssignmentIcon fontSize="large" /> Purchase Request & Approval Workflow
          </Typography>
          <Typography variant="body2" sx={{ color: '#9ca3af', mt: 0.5 }}>
            Create purchase requisitions, validate departmental budget limits & route multi-tier
            approvals
          </Typography>
        </Box>
        <Button
          ...
          <Table>
            <TableHead sx={{ background: '#1f2937' }}>
              <TableRow>
                ...
                <TableCell sx={{ color: '#9ca3af', fontWeight: 600 }}>Est Total Cost</TableCell>
              ...
              {requisitions.map((req) => (
                <TableRow key={req._id} sx={{ '&:hover': { background: '#1e293b' } }}>
                  ...
                  <TableCell sx={{ color: '#f8fafc' }}>
                    {req.requesterName || 'Procurement System'}
                  </TableCell>
                  <TableCell>
                    <Chip
                      ...
                      sx={{
                        ...
                        background:
                          req.priority === 'URGENT'
                            ? 'rgba(239, 68, 68, 0.2)'
                            : 'rgba(245, 158, 11, 0.2)',
                        color: req.priority === 'URGENT' ? '#f87171' : '#fbbf24',
                      }}
                    />
                  </TableCell>
                      ...
                      sx={{
                        ...
                        background:
                          req.status === 'APPROVED'
                            ? 'rgba(16, 185, 129, 0.2)'
                            : 'rgba(56, 189, 248, 0.2)',
                        color: req.status === 'APPROVED' ? '#34d399' : '#38bdf8',
                      }}
                    />
                  </TableCell>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}>
              ...
              >
                ...
                <option value="MEDIUM" style={{ background: '#111827' }}>
                  MEDIUM
                </option>
